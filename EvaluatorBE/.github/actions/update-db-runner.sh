...
if test "$STATUS"
then 
    export apiresponse=$(curl -X POST -v $GQL_URL -H 'Content-Type: application/json' -H "x-api-token:${access_token}" -d '{"query": "mutation($repoName: String!,$s3Path: String,$deployedUrl: String, $serviceName: String, $runId: String,$status: String){ updateUserAssessmentDetails(repoName:$repoName,deployedUrl:$deployedUrl,s3Path:$s3Path,serviceName:$serviceName,runId:$runId,status:$status){ok}} ","variables": {"serviceName": "'"${SERVICE_NAME}"'","repoName": "'"${REPO_NAME}"'", "runId": "'"${RUN_ID}"'", "status": "'"${STATUS}"'"}}' )
else
